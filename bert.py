...
class BertSelfAttention(nn.Module):
  ...
  def __init__(self, config):
    ...
    # print("no.of attention heads",self.num_attention_heads)
    # print("attention head_size",self.attention_head_size)
    # initialize the linear transformation layers for key, value, query
    self.query = nn.Linear(config.hidden_size, self.all_head_size)
    # print("config_hidden_size",config.hidden_size)
  ...
  def transform(self, x, linear_layer):
    ...
    if DEBUG: print("I'm in the transform",linear_layer)
    proj = linear_layer(x)
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    #First pass it through the linear layers
    # print(query.shape)
    # print(key.shape)
    # print(value.shape)
    # q  = self.transform(query,self.query)
    # print("q shape is",q.shape)
    # k = self.transform(key,self.key)
    if DEBUG: print("shape of query",query.shape)
    if DEBUG: print("shape of key", key.shape)
    if DEBUG: print("shape of Key.T", key.transpose(-2,-1).shape)

    S = torch.matmul(query,key.transpose(-2,-1))
    if DEBUG: print("Shape of dot product q*k.T",S.shape)
    # Step 2: Apply the mask to S
    #TODO: I'm doubtful whether this crctly works!!! Let's come back later.
    ...
    if DEBUG: print("after softmax",S.shape)
    ...
    if DEBUG: print("value shape",value.shape)
    S = torch.matmul(S,value)
    if DEBUG: print("After mul with value",S.shape)
    ...
    S = S.permute((0,2,1,3))
    if DEBUG: print("After shuffling",S.shape)
    s_shape = S.shape
    S = torch.reshape(S, (s_shape[0],s_shape[1],s_shape[2]*s_shape[3]))
    if DEBUG: print("After reshaping",S.shape)
...
class BertLayer(nn.Module):
  ...
  def __init__(self, config):
    ...
    # print("hiddent size", config.hidden_size)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    mah = self.self_attention(hidden_states=hidden_states,attention_mask=attention_mask)
    # Step 2: Apply add-norm layer using self.add_norm
    Fadd_norm = self.add_norm(hidden_states,mah, self.attention_dense,self.attention_dropout,self.attention_layer_norm)
    # Step 3: Get the output of feed forward layer using self.interm_dense
    if DEBUG: print("Output of first add&norm", Fadd_norm.shape)
    if DEBUG: print("Interm dense",self.interm_dense)
    out = self.interm_dense(Fadd_norm)
    if DEBUG: print("output of first dense in FFD:out",out.shape)
    # Step 4: Apply activation function to the output of feed forward layer using self.interm_af
    if DEBUG: print("Interm af",self.interm_af)
    out = self.interm_af(out)
    if DEBUG: print("output of ffd shape", out.shape)
    # Step 5: Apply another add-norm layer using self.add_norm
    if DEBUG: print("out_dense layer", self.out_dense)
    if DEBUG: print("out dropout layer",self.out_dropout)
    if DEBUG: print("out layer norm",self.out_layer_norm)
    out = self.add_norm(Fadd_norm,out, self.out_dense,self.out_dropout,self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    inputs_embeds = self.word_embedding(input = input_ids)
    ...
    pos_embeds  = self.pos_embedding(input = pos_ids)
